...
set_vars () {
    ...
    case "${TTY_COLS}" in "0"|"") TTY_COLS=130;; esac
    case "${TTY_LINES}" in "0"|"") TTY_LINES=30;; esac
...
interface_text () {
    ...
    printf "%s\n" "$_c_SORTED_VIDEO_DATA" |\
        ...
        '{
            ...
            sub(/  */, " ", ago)
            sub(/ $/, "", ago)
            agoListLength = split(ago, agoList, " ")
            prefixNum = agoList[1]
            multipleNum = agoList[2]
            if(multipleNum !~ /^[0-9]+$/ ) multipleNum = 1
            agoTotalHours = prefixNum * multipleNum
            title = $1
            gsub(" *", "", title)
            span = $3
            gsub(":", "", span)
...
parse_opt () {
	...
	case $opt in
        ...
        thumbnail-log) thumbnail_debug_log="${optarg:-}" ;;
